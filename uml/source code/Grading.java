...
public class Grading {
    ...
    * }
    * if (n % mod != 0) {
    * System.out.print("Initialization of " + filename +
    * " is not evenly completed,\n");
   ...
   private void setting(String table, String filename) {
      try {
         ...
         while (inputFile.hasNextLine()) { // Change hasNext to hasNextLine
            line = inputFile.nextLine();
            n++;
         }
         if (n % mod != 0) {
         ...
         inputFile = new Scanner(file);
         String[] result = new String[mod];
         int count = 0; // Counter to keep track of array index

         while (inputFile.hasNextLine()) { // Change hasNext to hasNextLine
            line = inputFile.nextLine();
            result[count++] = line; // Populate result array with data from file
            if (count == mod) { // If the array is filled, write it to the database
               Trans.write(table, result);
               System.out.println("Data written to database: " + Arrays.toString(result)); // Debugging print statement
               count = 0; // Reset counter for next set of data
            }
         }
         inputFile.close();
      } catch (FileNotFoundException e) {
         ...
         case 0:
            ...
            if (id.charAt(0) == 'd' && Trans.found("User", "id", "where id='" + id + "' and passwd='" + pw + "'")) {
               ...
               studentRole = new Student(id);
                  ...
                  && Trans.found("User", "id", "where id='" + id + "' and passwd='" + pw + "'")) {
               status = 11;
         ...
         case 4:
            ...
            do {
               ...
               if (0 <= k && k < item.length) {
                  if ((int) (index / Math.pow(10, k)) % 10 == 0)
                     index += (int) Math.pow(10, k);
                  else
                     index -= (int) Math.pow(10, k);
               }
