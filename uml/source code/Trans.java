...
public class Trans {
  static String url = "jdbc:mysql://localhost/final";
  ...
  public static boolean exist(String table) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (table.equals((rs.getObject(1) + "").toUpperCase()))
          flag = true;
      }
    } catch (Exception x) {
  ...
  public static String[] read(String table, String column, String condition) {
    // e.g., read("abc", "qty", "order by qty") for "select qty from abc order by
    // qty;"
    String[] results = { "" };
    try {
  ...
  public static void write(String table, String[] values) {
    // insertion, e.g., write ("abc", { "1-56592-488-6", "Database Management
    // System", "paper", "100" })
    // for "insert into abc Values ('1-56592-488-6', 'Database Management System',
    // 'paper', 100);"
    if (values.length < 1) {
